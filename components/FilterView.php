...
class FilterView extends Component
{
    ...
    public function __construct(ProductQuery $query, $config = [])
    {
        ...
        //Apply attributes
        $this->resultQuery->getFindByEavAttributesForViewTest($this->activeAttributes);
    ...
    public function getCategoryAttributesCallback()
    {
        ...
        $ea=[];
        if(Yii::$app->request->post('ea')){
        $ea = explode(',',Yii::$app->request->post('ea'));
        }
        //print_r($ea);die;
        foreach ($this->getRootCategoryAttributes() as $attribute) {
            ...
            foreach ($attribute['filters'] as $option) {
                $ignore=true;
if($ea && isset($option['id'])){

    if(in_array($option['id'],$ea)){
       $ignore=false;
    }
...
//print_r($newData);die;
    ...
    public function countRootAttributeProducts($attribute, $option)
    {
        ...
        if ($newData)
            $model->getFindByEavAttributesForViewTest($newData);
    ...
    public function getEavAttributes()
    {
        ...
        $queryCategoryTypes->select(ViewProduct::tableName() . '.type_id');
        $queryCategoryTypes->groupBy(ViewProduct::tableName() . '.type_id');
    ...
    public function getCategoryBrands()
    {
        ...
        $queryClone->addSelect(['brand_id', ViewProduct::tableName() . '.id']);
        $queryClone->joinWith([
