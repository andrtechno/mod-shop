                            ...
                            if ($filter['count'] > 0) {
                                ...
                                if ($attrData['type'] == \panix\mod\shop\models\Attribute::TYPE_COLOR) {
                                    ...
                                    if ($attrData['selectMany']) {
                                        ...
                                        echo Html::label(Html::radio('filter[' . $attrData['key'] . '][]', $checked, ['class' => '', 'value' => $filter['id'], 'id' => 'filter_' . $attrData['key'] . '_' . $filter['id']]) . $filter['title'] . (($checked) ? '' : $this->context->getCount($filter)), 'filter_' . $attrData['key'] . '_' . $filter['id'], ['class' => '']);
                                        echo '</div>';
                                    }
