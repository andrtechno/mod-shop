...
class ProductImage extends ActiveRecord
{
    ...
    public function afterDelete()
    {
        ...
        if ($ftp) {
            ...
            $deleted = $ftp->delete(Yii::$app->getModule('shop')->ftp['path'] . "/uploads/product/{$this->product_id}/{$this->filename}");

            $assetsList = $ftp->nlist(Yii::$app->getModule('shop')->ftp['path'] . "/assets/product/{$this->product_id}");
            foreach ($assetsList as $folder) {
                $deleted = $ftp->delete("{$folder}/{$this->filename}");
            }
        }
    ...
    public function get($size = false, array $options = [])
    {
        ...
        if (!$size) {
            if ($ftp) {
                return $module->ftp['host'] . "/" . $module->ftp['path'] . "/uploads/product/{$this->product_id}/{$this->filename}";
            }
            ...
            if ($ftp) {
                return $module->ftp['host'] . "/" . $module->ftp['path'] . "/assets/product/{$this->product_id}/{$size}/{$this->filename}";
            }
        }
        ...
        /*var_dump($img->getImage());die;
                echo Yii::getAlias("@app/web/assets/product/{$this->product_id}/{$size}/{$this->filename}");
                $upload = $ftp->fput("test/{$this->filename}", $img->getImage(), FTP_IMAGE);
                var_dump($upload);
        die;*/
...
//var_dump($img->getImage());die;
            ...
            //echo 'Loading to ftp';
            $pather = Yii::getAlias("@app/web/assets/product/{$this->product_id}/{$size}/{$this->filename}");
            if (file_exists($pather)) {
                $ftpPath = Yii::$app->getModule('shop')->ftp['path'] . "/assets/product/{$this->product_id}/{$size}";
                if (!$ftp->mkdir($ftpPath)) {
                    // echo "Не удалось создать директорию";
                }
                $upload = $ftp->put("$ftpPath/{$this->filename}", $pather, FTP_IMAGE);
                //var_dump($upload);
            } else {
                //echo $pather;
                //echo 'ERROR';
