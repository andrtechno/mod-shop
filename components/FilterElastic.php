    ...
    public function getActiveFilters()
    {
        ...
        if (!empty($activeAttributes)) {
            foreach ($activeAttributes as $attributeName => $value) {
                if (isset($this->attributesList[$attributeName])) { //changed from eavAttributes to attributes, else eavAttributes many load getEavAttributes function
                    $attribute = $this->attributesList[$attributeName];
...
//var_dump($this->route);die;
        if ($this->route === 'shop/catalog/view') {
           ...
           // $this->elasticQuery['bool']['must'][] = ["term" => ["categories" => $category->id]];
        ...
        // CMS::dump($this->data);die;

        //duplicate in ajax
        //$this->elasticQuery['bool']['must_not'][] = ["term" => ["availability" => Product::STATUS_ARCHIVE]];
        //$this->elasticQuery['bool']['must'][] = ["term" => ["switch" => 1]];
        //$this->elasticQuery['bool']['must'][] = ["term" => ["categories" => 5]];
...
//CMS::dump($search['aggregations']);die;
        ...
        if ($this->route != 'shop/brand/view') {
            ...
            if ($brands) {
                ...
                foreach ($brands as $brand) {
                    ...
                    $countBrand = (isset($optsBrands[$brand['brand_id']])) ? $optsBrands[$brand['brand_id']] : 0;
                    $show = true;
                    if (!Yii::$app->request->isAjax) {
                        $show = ($countBrand) ? true : false;
                    }
                    if ($show) {
    ...
    public function getAttributesCallback($test = [])
    {
        ...
        //CMS::dump($search['aggregations']['filtered']['buckets']);die;
        ...
        foreach ($this->_eavAttributes as $attribute) {
            ...
            foreach ($attribute->getOptions()->all() as $option) {
                $count = (isset($opts[$attribute->name][$option->id])) ? $opts[$attribute->name][$option->id] : 0;
...
//CMS::dump($this->elasticQuery);die;
    ...
    public function getElasticQuery($min = 1)
    {
        ...
        //CMS::dump($this->elasticQuery);
        //echo Json::encode($this->elasticQuery);
        //die;
        return $query;
    }
...
//CMS::dump($this->elasticQuery);die;
        ...
        $priceMax = $this->getMaxPrice();
        $priceMin = $this->getMinPrice();
...
//print_r($search);die;
    ...
    public function getEavAttributes()
    {
        ...
        if ($this->query) {
            ...
            $query->joinWith(['types type', 'options' => function ($q) {
                $q->orderBy(null);
            }]);
            if ($typesIds) {
            ...
            $query->distinct((Attribute::getDb()->driverName == 'pgsql') ? false : true);
            $query->useInFilter();
            //$query->sort();
            //$query->orderBy(null);
    ...
    public function getCategoryBrands()
    {
        ...
        $queryClone->addSelect(['brand_id', Brand::tableName() . '.name_' . Yii::$app->language . ' as name']);
        $queryClone->joinWith([
