...
class ElasticController extends ApiController
{
    ...
    public function actionIndex()
    {
        ...
        if ($route == 'shop/catalog/view') {
            $query['bool']['must'][] = ["terms" => ["categories" => [$param]]];
        }

        $query['bool']['must'][] = ["terms" => ["availability" => [1, 2, 3]]];
        $query['bool']['must'][] = ["term" => ["switch" => 1]];
        ...
        foreach ($active as $attribute_key => $items) {

            foreach ($items as $key => $count) {
                //$action = ($attribute_key == 'brand') ? 'brand_id' : 'options';
               // $query['bool']['must'][] = ["term" => ['options' => $key]];
        ...
        $filter2 = new FilterElastic(null, [
            'route' => $this->route,
            'q' => $query

        ]);
        $eav = $filter2->getEavAttributes();
        $startResult = $filter->getElasticQueryCallback([])->search(null, ['size' => 0,'filter_path'=>'aggregations']);
        print_r($startResult);die;
        $startResult = $filter->getElasticQuery(1)->search(null, ['size' => 0]);
        ...
        // print_r($startResult['aggregations']['options']['buckets']);die;
        $result = $filter->getAttributesCallback($data);


        //CMS::dump($query);die;
        // $result = $test->getAttributes($query);
